...
# configuration generale utilisee par les services kubernetes
...
# on pointe sur l'adresse du master
KUBE_MASTER=\"--master=http://192.168.50.130:8080\"
" > /etc/kubernetes/config


# configuration du kubelet on specifie le master et on donne un nom au noeud

echo "
KUBELET_ADDRESS=\"--address=0.0.0.0\"
KUBELET_PORT=\"--port=10250\"
# change the hostname to this host's IP address
# on donne un nom a notre noeud
KUBELET_HOSTNAME=\"--hostname_override=192.168.50.131\"
# on pointe sur l'adresse du master
KUBELET_API_SERVER=\"--api_servers=http://192.168.50.130:8080\"
# on pointe sur l'adresse du service DNS et on donne un nom a celui-ci
...
# on verifie que l'adresse ip est differente pour chaque noeud

ip a | grep flannel | grep inet

echo "INSTALLATION DU NODE-01 TERMINEE"
